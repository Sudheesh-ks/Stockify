...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      setRecentProducts(data.recent.products.map((p: ProductTypes) => [p.name, p.quantity, `₹${p.price}`]));

      setRecentCustomers(data.recent.customers.map((c: CustomerTypes) => [c.name, c.mobile]));
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-8 p-6 md:p-8 min-h-screen bg-transparent">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard title="Total Products" value={stats.products} Icon={Package} link="/products" />
          <StatCard title="Total Customers" value={stats.customers} Icon={Users} link="/customers" />
          <StatCard title="Total Sales" value={stats.sales} Icon={IndianRupee} link="/sales" />
        </div>

        {/* TABLES */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <DashboardTable title="Recent Customers" headers={['Name', 'Mobile']} data={recentCustomers} />

          <DashboardTable title="Recent Sales" headers={['Item', 'Customer', 'Qty', 'Amount']} data={recentSales} />

          <DashboardTable title="Latest Stock" headers={['Item', 'Stock', 'Price']} data={recentProducts} />
        </div>
      </div>
    </DashboardLayout>
  );
