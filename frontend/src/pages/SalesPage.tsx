...
import { Plus, ShoppingCart, Trash2, Package, Eye, FileText, Calendar, User } from "lucide-react";
...
const SalesPage = () => {
    ...
    const handleDelete = (id: string) => {
        ...
        const productNames = saleToDelete?.items.map(i => i.productName).join(", ");
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-400 text-sm">Professional multi-product invoicing system.</p>
                    </div>
                    <button
                        ...
                        <Form className="space-y-6">
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setFieldValue("items", [...values.items, { productId: "", quantity: 1, price: 0, _tempName: "" }])}
                                    ...
                                    {({ remove }) => (
                                        <div className="space-y-3">
                                            {values.items.map((item, index) => (
                                                <div key={index} className="group relative flex flex-col gap-3 p-4 bg-[#0d1117]/50 rounded-xl border border-[#1a1f2a] hover:border-emerald-500/30 transition-all">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        <SearchableSelect
                                                            ...
                                                            onSelect={(opt) => {
                                                                setFieldValue(`items.${index}.productId`, opt?.value || "");
                                                                setFieldValue(`items.${index}.price`, opt?.original?.price || 0);
                                                                setFieldValue(`items.${index}._tempName`, opt?.label || "");
                                                            }}
                                                            error={errors.items?.[index]?.productId}
                                                            touched={touched.items?.[index]?.productId}
                                                        />
                                                        ...
                                                        <div className="flex gap-4">
                                                            ...
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="text-sm font-bold text-emerald-400 pt-2">
                                                                    ₹{(values.items[index].price * (values.items[index].quantity || 0)).toFixed(2)}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-[#1a1f2a]">
                                ...
                                <SearchableSelect
                                    ...
                                    onSelect={(opt) => setFieldValue("customerName", opt?.value || "")}
                                />
                            </div>
                            ...
                            <div className="bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/10 flex justify-between items-center">
                                ...
                                <span className="text-2xl font-black text-emerald-400">
                                    ₹{values.items.reduce((total, item) => total + (item.price * (item.quantity || 0)), 0).toFixed(2)}
                                </span>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 p-4 bg-[#0d1117] rounded-xl border border-[#1a1f2a]">
                            <div className="space-y-1">
                                ...
                                <div className="text-sm font-medium text-white">
                                    {new Date(selectedSale.date).toLocaleDateString(undefined, { dateStyle: 'long' })}
                                </div>
                            </div>
                            <div className="space-y-1">
                                <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-widest">
                                    ...
                                    <tbody className="divide-y divide-[#1a1f2a]">
                                        {selectedSale.items.map((item, idx) => (
                                            <tr key={idx} className="bg-[#0d1117]/50">
                                                ...
                                                <td className="px-4 py-3 text-center text-gray-400">x{item.quantity}</td>
                                                <td className="px-4 py-3 text-right text-gray-400">₹{item.price.toFixed(2)}</td>
                                                <td className="px-4 py-3 text-right text-emerald-400 font-bold">₹{(item.price * item.quantity).toFixed(2)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot className="bg-[#151b23]/50">
                                        <tr>
                                            <td colSpan={3} className="px-4 py-4 text-right text-gray-400 font-bold uppercase tracking-wider text-xs">Grand Total</td>
                                            <td className="px-4 py-4 text-right text-xl font-black text-emerald-400">₹{selectedSale.totalAmount.toFixed(2)}</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
