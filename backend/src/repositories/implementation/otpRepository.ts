...
export class OtpRepository extends BaseRepository<OtpDocument> implements IOtpRepository {
  ...
  async storeOtp(email: string, data: Partial<OtpTypes>): Promise<void> {
    await this.findOneAndUpdate({ email }, { ...data, createdAt: new Date() }, { upsert: true, new: true });
  }

  async getOtp(email: string, otp?: string, purpose?: string): Promise<OtpTypes | null> {
