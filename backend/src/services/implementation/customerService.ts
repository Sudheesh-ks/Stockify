...
export class CustomerService implements ICustomerService {
    ...
    async createCustomer(userId: string, customer: {name: string, address: string, mobile: string}): Promise<CustomerDTO> {
        const created = await this._customerRepository.createCustomer({ ...customer, userId: userId as any });
        return toCustomerDTO(created);
    }

    async updateCustomer(id: string, userId: string, customer: {name?: string, address?: string, mobile?: string}): Promise<CustomerDTO> {
    ...
    async getAllCustomers(userId: string, search?: string, page?: number, limit?: number): Promise<{ customers: CustomerDTO[], totalCount: number, totalPages: number, currentPage: number }> {
